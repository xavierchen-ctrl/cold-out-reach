...
第 2 頁：OUR HISTORY — 潮網的成長歷程（時間軸設計）
- 2010：潮網的起點（數位廣告代理起家）
- 2012：進軍國際（佈局海外市場）
- 2014：走入 ADTECH（自建廣告技術平台 PUNWAVE）
- 2018：佈局 MARTECH（推出 ORCA CDP & MA+）
- 2025：持續建構行銷生態圈（整合 AI、數據、媒體全鏈路）
- 設計：橫向時間軸，每個節點用圓形 + 年份標示，搭配簡短說明

第 3 頁：SERVICE — 數據驅動行銷，四大服務類別（2×2 卡片格局）
卡片一「數位廣告」
...
- 核心理念：以 AI 人工智慧、大數據分析為核心
- 系統架構（流程圖）：
  PUNWAVE RTB（即時競價）
  → PUNWAVE DMP（資料整合 / 資料標籤分類 / 資料分析）
  → PUNWAVE ATD
  → PUNWAVE AI Smart Manager（24小時即時優化動態預算分配）
  → 串接 Facebook / Google / PUNWAVE DSP 廣告平台
- 設計：深藍底色，白色節點箭頭流程圖，強調 AI 自動化

第 5 頁：Excel 報表範例（每日數據）
- 標題：每日廣告數據追蹤報表
- 模擬 Excel 表格樣式（以表格元件呈現），欄位包含：
  日期 / 廣告策略 / 曝光數(Impression) / 點擊數(Click) / CTR / CPC / 成本 / 花費
- 展示多種廣告策略（如品牌曝光、再行銷、關鍵字）的每日數據對比
- 說明：數據格式為 Excel，每日更新，協助客戶掌握廣告投放狀況

第 6 頁：Excel 報表範例（媒體策略分析）
- 標題：媒體數據策略表現 + 顧問分析 Finding
- 模擬 Excel 報表，呈現各媒體（Meta / Google / YouTube / LINE）的整體表現彙整
- 下方區塊：顧問 Finding 分析，以重點條列說明當期廣告洞察、優化建議、下期策略調整方向
- 設計：上半部表格、下半部文字框（橘色標題列）

第 7 頁：圖像報表範例（一）
- 標題：客製化圖像分析報表
- 展示三種圖表類型（並排卡片）：
  ① TA 分析：受眾年齡/性別/興趣分佈圓餅圖 + 長條圖
  ② 逐週表現：折線圖，顯示曝光、點擊、轉換的週別趨勢
  ③ 競價分析：CPM/CPC 競爭態勢，與產業均值對比
- 說明：依媒體與產業特性客製化圖像報表

第 8 頁：圖像報表範例（二）
- 標題：多媒體整合成效報表
- 展示三組圖表（並排）：
  ① 策略表現：各廣告策略成效比較（堆疊長條圖）
  ② Google 關鍵字月別表現：月份 vs 點擊/CPC 折線圖
  ③ Criteo 廣告成效：再行銷 ROAS 與轉換數趨勢圖
- 設計：圖表搭配簡短數據標註，視覺化呈現

第 9 頁：雲端素材表範例
- 標題：Meta 廣告素材雲端管理表
- 上半部：模擬 Google Sheets 雲端素材追蹤表（欄位：貼文連結 / 廣告目標 / 素材預覽縮圖 / 格式 / 上線日期 / 狀態）
- 下半部：8 種 Meta 廣告格式說明表：
  單圖廣告 / 輪播廣告 / 影片廣告 / 限時動態廣告 / Reels 廣告 / 即時體驗廣告 / 精選集廣告 / 動態廣告
- 設計：雲端圖示 + 表格清晰排版
