...
_NOISE_WORDS = {
    ...
    '忘記密碼', '立即登入', '立即註冊', '重設密碼',
    # 展覽網站導覽/功能頁（非公司名稱）
    '參展商列表', '參展資訊', '參展說明', '參展商行事曆', '參展商服務',
    '參展商免費服務', '參展商加值服務', '參展商攤位活動', '攤位活動',
    '洽遊外商人士來臺觀展', '觀展資訊', '展覽資訊', '展期資訊',
    '媒體中心', '媒體資源', '新聞稿', '新聞發佈',
    '交通資訊', '場地資訊', '展場地圖', '住宿資訊',
}
    ...
    async with httpx.AsyncClient(headers=HEADERS, timeout=20, follow_redirects=True) as client:
        ...
        if _needs_batch_pw:
            ...
            _batch_pw_html = await asyncio.wait_for(
                _batch_fetch_with_playwright(_batch_urls, wait_ms=800, page_timeout=8000),
                timeout=min(len(_batch_urls) * 10 + 20, 300),  # 最多 5 分鐘
            )
